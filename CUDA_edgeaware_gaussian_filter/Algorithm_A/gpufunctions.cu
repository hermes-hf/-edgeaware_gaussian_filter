...
void image_kernel_call_horizontal(uchar4 *auximage,uchar4 *outputimage,float sigma_r,float sigma_h,thrust::complex <float> *constant, uchar4  *img,int width, int height, int channels ,int window_w, int window_h){
    ...
    gaussian_filter_kernel_horizontal_causal<<<grid,block,(window_h*(window_w+sharedpad))*sizeof(uchar4),stream1>>>(auximage,sigma_h,s_quotient,constant,img,width,  height, channels,window_w,window_h, sharedpad);
    gaussian_filter_kernel_horizontal_anticausal<<<grid,block,(window_h*(window_w+sharedpad))*sizeof(uchar4),stream2>>>(outputimage,sigma_h,s_quotient,constant,img,width,  height, channels,window_w,window_h,sharedpad);
...
void image_kernel_call_vertical(uchar4 *auximage,uchar4 *outputimage,float sigma_r,float sigma_h,thrust::complex <float> *constant, uchar4  *img,int width, int height, int channels ,int window_w, int window_h){
    ...
    gaussian_filter_kernel_vertical_causal<<<grid,block,(window_h*(window_w+0))*sizeof(uchar4)>>>(outputimage,sigma_h,s_quotient,constant,img,width,  height, channels,window_w,window_h, 0);
    gaussian_filter_kernel_vertical_anticausal<<<grid,block,(window_h*(window_w+0))*sizeof(uchar4)>>>(auximage,sigma_h,s_quotient,constant,img,width,  height, channels,window_w,window_h, 0);
    cudaDeviceSynchronize();
    dim3 grid2(ceil(width*height/window_h),1,1);
    dim3 block2(window_h,1,1);
    vecAdd<<<grid2,block2>>>(auximage,outputimage,outputimage,width*height);
...
void image_filter2d(float *exectime,float sigma_r,float sigma_h,uchar4 *inputimage,int width,int height, int channels,int window_w, int window_h){
    ...
    for(i=0;i<num_it ;i++){
        ...
        image_kernel_call_horizontal(auximage,device_out,sigma_r,sigma_h,device_constant,device_in,width,height,channels,window_w,window_h);
        aux = device_out;
        ...
        compute_constants(constant,sigma);
        cudaMemcpy(device_constant, constant, 13*sizeof(thrust::complex<float>), cudaMemcpyHostToDevice);
        image_kernel_call_vertical(auximage,device_out,sigma_r,sigma_h,device_constant,device_in,width,height,channels,window_w,window_h);
        aux = device_out;
        device_out = device_in;
        device_in = aux;
        sigma = sigma*ratio;
